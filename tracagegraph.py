...
def lire_fichier_pos (fichier) :
    ...
    return (x, y, z, nbter - 1)


def calcul_angles_plans (x, y, z) :
    ...
    for i in range(1, len(x) - 1) :
        if x[i - 1] == None or x[i] == None or x[i + 1] == None :
            a.append (None)
            b.append (None)
            c.append (None)
            d.append (None)
        else :
            a.append((y[i] - y[i - 1]) * (z[i + 1] - z[i - 1]) - (z[i] - z[i - 1]) * (y[i + 1] - y[i - 1]))
            b.append((z[i] - z[i - 1]) * (x[i + 1] - x[i - 1]) - (x[i] - x[i - 1]) * (z[i + 1] - z[i - 1]))
            c.append((x[i] - x[i - 1]) * (y[i + 1] - y[i - 1]) - (y[i] - y[i - 1]) * (x[i + 1] - x[i - 1]))
            d.append(- a[i - 1] * x[i] - b[i - 1] * y[i] - c[i - 1] * z[i])
    
    nb = 0
    maxisi = []
    
    for i in range(1, len(a)):
        if a[i - 1] == None or a[i] == None or x[i + 2] == None :
            if nb != 0 :
                maxisi.append (nb)
                nb = 0
        ...
        alpha = math.acos ((a[i - 1] * a[i] + b[i - 1] * b[i] + c[i - 1] * c[i]) / (math.sqrt (a[i - 1] * a[i - 1] + b[i - 1] * b[i - 1] + c[i - 1] * c[i - 1]) * math.sqrt (a[i] * a[i] + b[i] * b[i] + c[i] * c[i]))) * 180 / math.pi
        
        if a[i - 1] * x[i + 2] + b[i - 1] * y[i + 2] + c[i - 1] * z[i + 2] + d[i - 1] < 0 :
            angles.append (- alpha)
        else:
            angles.append (alpha)
...
def list_distances (x, y, z, pas) :
    ...
    for i in range (len (xf) - pas) :
        distances.append (distance3d (xf [i], yf [i], zf [i], xf [i + pas], yf [i + pas], zf [i + pas]))
...
def creer_paves (donnes, discretisation) :
...
def Exe_rela (s, maxisc, lnbter, x, y, z, pavesa, discretisationd, degenerescenced, k, quorum) :
    ...
    if k != - 1 :
        ...
        while len (p) != 0 :
            ppre = p
            cptnnr += 1
            ld =  list_distances(x, y, z, cptnnr)
            pavesd = creer_paves (ld, discretisationd)
            pavesd = paves_agrandis (pavesd, degenerescenced)
            v = VectPf (p)
            p = Dual (v)
...
def main (fichier, discretisationa, degenerescencea, k) :
    ...
    (x, y, z, nbter) = lire_fichier_pos (fichier)
    lnbter = [nbter]
    tup = calcul_angles_plans (x, y, z)
    s = tup[0]
    maxisi = tup[1]
    maxisc = [maxisi[0]]
...
def main2 (fichier, discretisationa, degenerescencea, discretisationd, degenerescenced, k) :
    ...
    pavesa = creer_paves (s, discretisationa)
    pavesa = paves_agrandis (pavesa, degenerescencea)
    (result, maxk) = Exe_rela (s, maxisc, lnbter, x, y, z, pavesa, discretisationd, degenerescenced, k, 1)
    if k != - 1 :
...
def cut (p, maxisc, lnbter, k, quorum):
    ...
    for ens in p :
        ...
        if nbq < quorum or len (res[len (res) - 1]) == 0:
            res.pop ()
...
def main3 (list_fichiers, discretisationa, degenerescencea, k, quorum) :
    ...
    for c in list_fichiers:
        (x, y, z, nbter) = lire_fichier_pos (c)
        lnbter.append (nbter)
        tup = calcul_angles_plans (x, y, z)
        s += tup[0]
        maxisi += tup[1]
...
def prog_plot () :
    ...
    plotf (l, discretisationa, degenerescencea, discretisationd, degenerescenced, quorum) 
...
### Fonction d'affichage graphique pour comparaison d'avant et d'après fusion.
    

def plotf (list_fichiers, discretisationa, degenerescencea, discretisationd, degenerescenced, quorum) :
    ...
    if k != -1 :
        p = cut (p, maxisc, lnbter, k, quorum)
    ...
    while len (p) != 0 :    
        ...
        for i in range (0, 101) :
            x.append (i / 100)
            ya.append (len (p))
            p = fusion_motifs_similaires (p, i / 100)
            yb.append (len (p))
            p = copy_result.copy ()
        ...
        pavesa = creer_paves (s, discretisationa)
        pavesa = paves_agrandis (pavesa, degenerescencea)
        ld =  list_distances(lx, ly, lz, k - 1)
        pavesd = creer_paves (ld, discretisationd)
        pavesd = paves_agrandis (pavesd, degenerescenced)
        v = VectPf (p)
        p = Dual (v)
...
def tempspdb (taille):
    ...
    for degenerescenced in range (1, 6):
        print("\n\n")
        for degenerescencea in range (1, 6):
            if c == 1 :
                (t, maxk) = main (l[0], discretisationa, degenerescencea, taille)
            elif c == 2 :
                (t, maxk) = main2 (l[0], discretisationa, degenerescencea, discretisationd, degenerescenced, taille)
            elif c == 3 :
                (t, maxk) = main3 (l, discretisationa, degenerescencea, taille, quorum)
            elif c == 4 :
                (t, maxk) = main4 (l, discretisationa, degenerescencea, discretisationd, degenerescenced, taille, quorum)
            else :
                (t, maxk) = main_final (l, discretisationa, degenerescencea, discretisationd, degenerescenced, taille, quorum, seuil)
