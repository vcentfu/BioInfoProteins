...
def main (fichier, discretisationa, degenerescencea, k) :
    ...
    for ens in result :
        if len (ens) != 0 :
            nb += 1
            new_fichier = "./logs/{}-motif{}.pdb".format (maxk, nb)
...
def prog () :
    ...
    if c in {3, 4, 5} :
        nbplu =  len ([k for k in os.listdir ("./instances") if ".pdb" in k])
    ...
    for i in range (nb) :
        lp = [k for k in os.listdir ("./instances") if ".pdb" in k]
        ...
        while not os.path.exists ("./instances/" + el) :
